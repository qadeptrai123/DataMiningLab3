...
Theo Bổ đề \ref{lemma:4}, tập các tọa độ "tốt" chiếm ít nhất một nửa số lượng các tọa độ trong tập tối ưu con . Do , ta có chặn dưới cho tỷ lệ điểm tốt trong :
\begin{equation*}
...
\begin{enumerate}
    \item \textbf{Cận dưới:} \\
    Theo định nghĩa, $Q'_{ij}$ là tập con của $P_{ij}$ có kích thước $(1 - \alpha)m_i$ tối thiểu hóa tổng bình phương khoảng cách đến tâm của nó. Vì $\mathcal{N}_{ij}(u)$ cũng là một tập con của $P_{ij}$ với cùng kích thước $(1 - \alpha)m_i$ (được xác định tại Bước 5 của Thuật toán \ref{alg:fast_sampling}), chi phí phân cụm của $\mathcal{N}_{ij}(u)$ không thể nhỏ hơn chi phí tối ưu của $Q'_{ij}$. Do đó:
    \begin{equation*}
        \delta^2(\mathcal{N}_{ij}(u), \overline{\mathcal{N}_{ij}(u)}) \geq \delta^2(Q'_{ij}, \overline{Q'_{ij}})
    \end{equation*}

    \item \textbf{Cận trên:} \\
    Ta áp dụng các tính chất của trọng tâm và định nghĩa về lân cận gần nhất để biến đổi biểu thức:
    \begin{align}
    ...
    Trong đó:
    \begin{itemize}
        \item Dòng 2 đúng vì $\mathcal{N}_{ij}(u)$ là tập hợp các điểm trong $P_{ij}$ gần $u$ nhất, nên tổng khoảng cách từ nó đến $u$ nhỏ hơn hoặc bằng tổng khoảng cách từ bất kỳ tập nào khác cùng kích thước (như $Q'_{ij}$) đến $u$.
        \item Dòng 3 sử dụng Bổ đề \ref{lemma:1} (phân rã khoảng cách).
    \end{itemize}
...
\begin{enumerate}
    ...
    Theo Bổ đề \ref{lemma:5}, tồn tại $u \in U_{ij} \cap G_2^{ij}$. Từ Bổ đề \ref{lemma:5}, trọng tâm $\overline{Q'_{ij}}$ nằm trong khoảng $[u - l_{ij}, u + l_{ij}]$ với độ dài $l_{ij}$ được ước lượng từ tập lân cận $\mathcal{N}_{ij}(u)$.
    ...
    \item
    Thay thế các giá trị tham số $\epsilon' = \sqrt{\frac{\epsilon}{48}}$ và chặn trên của $l_{ij} \leq 2\sqrt{\frac{6\delta^2(Q'_{ij}, \overline{Q'_{ij}})}{(1-\alpha)m_i}}$ (từ Bổ đề \ref{lemma:5}), ta có:
    \begin{equation*}
...
Trước hết, ta xét tập hợp $I'_{ij}$ bao gồm $(1-\alpha)m_i$ tọa độ trong $P_{ij}$ gần nhất với một điểm mẫu $u' \in U'_{ij}$. Theo Hệ quả \ref{cor:2}, với xác suất hằng số, tồn tại $u'$ sao cho $u'$ nằm rất gần tâm $\overline{Q'_{ij}}$:
\[ \delta(u', \overline{Q'_{ij}}) \leq \sqrt{\frac{\epsilon \delta^2(Q'_{ij}, \overline{Q'_{ij}})}{2(1-\alpha)m_i}} \]
Áp dụng Bổ đề \ref{lemma:1} và tính tối ưu của trọng tâm, ta có chặn chi phí cho $I'_{ij}$ :
\begin{align*}
...
Trong Bước 10 của Thuật toán \ref{alg:fast_sampling}, tập $I_{ij}$ được chọn là tập có chi phí nhỏ nhất trong số các ứng viên. Do đó, chi phí của nó không vượt quá chi phí của $I'_{ij}$ :
...
Đặt tỷ lệ trùng lặp $\zeta = \frac{|S|}{|I_{ij}|}$. Ta áp dụng Bổ đề \ref{lemma:2} (về quan hệ giữa chi phí của tập con và tập cha):

\textit{2a: Khoảng cách từ tâm $\overline{I_{ij}}$ đến tâm giao $\overline{S}$.}
Áp dụng Bổ đề \ref{lemma:2} với $J = I_{ij}$ và $J_1 = S$:
\begin{align*}
...
Tương tự, áp dụng Bổ đề \ref{lemma:2} với $J = Q_{ij}$ và $J_1 = S$. Gọi $\zeta' = |S|/|Q_{ij}|$. Phần bù là các điểm thuộc $Q_{ij}$ nhưng không thuộc $I_{ij}$, kích thước tối đa là $\alpha m_i$. Ta có :
...
Tiếp tục áp dụng Bổ đề \ref{lemma:1} cho từng số hạng:
\begin{itemize}
...
\item \textbf{Kết hợp với Bổ đề \ref{lemma:6}}
...
Áp dụng Bổ đề \ref{lemma:1}:
\[ \delta^2(P^*_{ij}, c_{ij}) = \delta^2(P^*_{ij}, \overline{P^*_{ij}}) + |P^*_{ij}| \delta^2(\overline{P^*_{ij}}, c_{ij}) \]

Sử dụng kết quả từ Bổ đề \ref{lemma:7}, ta có chặn trên cho khoảng cách giữa tâm tối ưu $\overline{P^*_{ij}}$ và tâm thuật toán $c_{ij}$:
...
Thay thế phương trình Bổ đề \ref{lemma:1} vào bất đẳng thức trên:
\begin{align*}
...
\begin{enumerate}
    ...
    Theo Bổ đề \ref{lemma:4}, tập hợp các tọa độ tốt $G^2_{ij}$ chiếm ít nhất một nửa số lượng các tọa độ trong tập tối ưu con $Q'_{ij}$. Do đó, tỷ lệ phần tử tốt trong toàn bộ $P_{ij}$ là:
    ...
    % TODO prove 
    Sử dụng bất đẳng thức $1-x \leq e^{-x}$ ta có:
...
\begin{proof}
Theo Bổ đề \ref{lemma:8} và \ref{lemma:9}, với xác suất ít nhất $1 - \frac{\epsilon_1}{m^2 d \log^2(m \Delta_{\max}^2)}$, các điều kiện sau đây đồng thời xảy ra đối với tập mẫu ngẫu nhiên $S_{ij}$:
\begin{enumerate}
...
\begin{align*}
    ...
    &\leq \left( (1+\epsilon_1) \frac{|S_{ij}|}{m_i} |\mathcal{B}_u^l| \right) \cdot (1+\epsilon_1)^{l+1} \quad \text{(từ Bổ đề \ref{lemma:8})} \\
...
Từ Bổ đề \ref{lemma:8}, ta có $|\mathcal{B}_u^l| \leq \frac{m_i}{|S_{ij}|(1-\epsilon_1)} |\mathcal{B}_u^l \cap S_{ij}|$. Thay thế vào bất đẳng thức:
\begin{align*}
...
Theo Bổ đề \ref{lemma:4} và Bổ đề \ref{lemma:5}, với xác suất hằng số, tồn tại ít nhất một tọa độ $u_1 \in U'_{ij}$ nằm rất gần trọng tâm của tập $Q'_{ij}$ (tập con của $Q_{ij}$ có chi phí nhỏ nhất với kích thước $(1-\alpha)m_i$). Cụ thể:
\[ \delta^2(u_1, \overline{Q'_{ij}}) \leq \frac{\epsilon_1 \delta^2(Q'_{ij}, \overline{Q'_{ij}})}{|Q'_{ij}|} \]
Sử dụng Bổ đề \ref{lemma:1}, ta liên hệ chi phí của tập các điểm lân cận $\mathcal{N}_{ij}(u_1)$ với chi phí tối ưu:
...
Gọi $c_{ij}$ là tọa độ được bộ ước lượng $\omega$ chọn ở Bước 11 của Thuật toán \ref{alg:fast_estimation}. Do $c_{ij}$ tối thiểu hóa $\omega$ trên $U'_{ij}$, ta có $\omega(c_{ij}) \leq \omega(u_1)$. Kết hợp với các chặn của bộ ước lượng từ Bổ đề \ref{lemma:10}:
...
Áp dụng Bổ đề \ref{lemma:6} (đã được chứng minh cho Fast-Sampling và mở rộng cho Fast-Estimation), ta có các chặn sau cho từng thành phần khoảng cách:
...
Kết hợp lại, bình phương tổng các khoảng cách với điều kiện $\epsilon < 0.5$ và $\alpha < 1/3$, ta thu được chặn cuối cùng:
\begin{align*}
...
Dựa vào Bổ đề \ref{lemma:7} (được chứng minh dựa trên kết quả của Bổ đề \ref{lemma:11} về khoảng cách giữa $\overline{I_{ij}}$ và $\overline{Q_{ij}}$), ta có chặn trên cho khoảng cách giữa các tâm:
...
Sự thành công của Fast-Estimation phụ thuộc vào độ chính xác của bộ ước lượng $\omega(u)$. Điều này được đảm bảo bởi Bổ đề \ref{lemma:8} và Bổ đề \ref{lemma:9} thông qua việc lấy mẫu ngẫu nhiên.
...
\begin{algorithm}[H]
\caption{Fast-Filtering (cụ thể)}
\label{alg:fast_filtering}
\begin{algorithmic}[1]
\Require Một bài toán $k$-means $(P, k, d)$, một tập các phân vùng $(P_1, P_2, \dots, P_k)$ với tỷ lệ lỗi $\alpha$, tham số $0 < \epsilon < 1/3$.
\Ensure Một tập $C \subset \mathbb{R}^d$ các tâm sao cho $|C| \leq k$.

\For{mỗi $i \in [k]$}
    \State Lấy mẫu ngẫu nhiên và độc lập một tập $U_i$ từ $P_i$ với kích thước $R_1 = O\left(\frac{\log k}{1-2\alpha}\right)$.
    \State Lấy mẫu ngẫu nhiên và độc lập một tập $S_i$ từ $P_i$ với kích thước:
    \[ R_2 = O\left( \frac{\log(m^3 d \log^3(m\Delta^2)/\epsilon^2) \log(m\Delta^2)}{\alpha\epsilon^4} \right) \]
    và gán cho mỗi điểm trong $S_i$ một trọng số $\frac{m_i}{|S_i|}$.
    \State Xây dựng một bộ ước lượng $\omega$ sao cho với mọi $u \in U_i$:
    \[ \omega(u) = \sum_{p \in S_i \setminus \mathcal{F}(u)} \frac{m_i}{|S_i|} \delta^2(p, u) \]
    trong đó $\mathcal{F}(u)$ là tập hợp $(1 + 3\epsilon_1)\alpha |S_i|$ điểm xa nhất từ $S_i$ tới $u$, với $\epsilon_1 = \epsilon/126$.
    \State Tìm tâm ứng viên $c_i = \arg \min_{u \in U_i} \omega(u)$.
    \State Gọi $I_i$ là tập hợp gồm $(1 - 2\alpha - 20\alpha\epsilon_1)m_i$ điểm trong $P_i$ gần $c_i$ nhất.
    \State $\hat{c}_i = \overline{I_i}$
\EndFor
...
Gọi tập $G_2(P^*_i) = \{x \in P^*_i : \delta^2(x, c^*_i) \leq 2 \delta^2(P^*_i, c^*_i) / |P^*_i|\}$. Theo Bổ đề \ref{lemma:4} , $|P_i \cap P^*_i| \geq (1-\alpha)\max(|P_i|, |P^*_i|)$, ta suy ra:
...
Với kích thước mẫu $R_1 = \Theta\left(\frac{1}{1-2\alpha}\log\left(\frac{k}{\eta}\right)\right)$, xác suất để tập $U_i$ chứa ít nhất một điểm tốt $u_i \in G_2(P^*_i)$ là rất cao (xem chứng minh Hệ quả~\ref{cor:3}).
...
%         % 1. Trọng tâm Q' (đã chuẩn hóa về gốc tọa độ 0,0)
%         \coordinate (Centroid) at (0,0);
        
%         % 2. Vẽ vòng tròn ngưỡng với bán kính xác định: R = 2 * r_avg = 1.97cm
%         \draw[dashed, thick, grayline] (Centroid) circle (1.97cm);
%         % \node[grayline, above right] at (1.4, 1.4) {Ngưỡng $\mu \cdot \text{Var}$};

%         % 3. Các điểm "Tốt" G_{ij} (Tọa độ đã trừ đi centroid để tâm là 0,0)
%         \foreach \p in {
%             (0.34, 0.48), (-0.56, -0.42), (0.84, -0.52), (-0.86, 0.58), 
%             (0.14, -1.22), (-1.26, -0.02), (0.64, 0.68), (-0.36, 1.08), 
%             (1.24, 0.08), (-0.16, -0.72)
%         } {
%             \fill[goodgreen] \p circle (2pt);
%         }

%         % 4. Các điểm "Xa" (Nằm ngoài ngưỡng 1.97cm)
%         \foreach \p in {(1.8, 1.2), (-1.4, -1.6), (2.0, -0.6)} {
%             \fill[badred] \p circle (2pt);
%         }

%         % 5. Điểm trọng tâm trung tâm
%         \fill[black] (Centroid) circle (2.8pt);
%         \node[below=5pt, font=\large] at (Centroid) {$\overline{Q'_{ij}}$};

%         % 6. Vẽ Vector bán kính minh họa chính xác 2 x r_avg
%         \draw[<->, thick, black] (Centroid) -- (215:1.97) 
%             node[midway, above, rotate=35, font=\scriptsize, yshift=2pt] {Bán kính};

%         % 7. Chú thích (Legend)
%         \begin{scope}[shift={(2.8, 0.8)}]
%             \draw[draw=black, fill=white, thin] (-0.2,-1) rectangle (2.6, 0.5);
%             \fill[goodgreen] (0,0) circle (2pt) node[right, black, font=\small] {: $G^{\mu}_{ij}$ (Tọa độ tốt)};
%             \fill[badred] (0,-0.4) circle (2pt) node[right, black, font=\small] {: Điểm nằm xa};
          
%         \end{scope}

%         % Nhãn tên tập hợp
%         \node[font=\LARGE] at (-2.3, 2.3) {$Q'_{ij}$};

%     \end{tikzpicture}
%     \caption{Bổ đề \ref{lemma:4} với bán kính vùng tốt được xác định chính xác bằng $2 \times$ khoảng cách trung bình cụm.}
% \end{figure}
...
% Với kích thước mẫu $R_2$ được chọn đủ lớn, Hệ quả 4 (Corollary 4) dựa trên Bổ đề \ref{lemma:10} cho thấy bộ ước lượng xấp xỉ chính xác chi phí thực tế.
...
Điều này đảm bảo rằng tâm được chọn $c_i$ (và tập hợp sau lọc $I_i$) có chất lượng tốt, làm tiền đề cho Bổ đề \ref{lemma:12}. 

\textbf{3. Tổng chi phí}

Ta đánh giá tổng chi phí của nghiệm cuối cùng $C = \{\overline{I_1}, \dots, \overline{I_k}\}$. Tổng chi phí là tổng chi phí của từng cụm tối ưu $P^*_i$ được gán cho tâm tương ứng $\overline{I_i}$:
\[ \delta^2(P, C) \leq \sum_{i=1}^k \delta^2(P^*_i, \overline{I_i}) \]
Sử dụng kết quả trực tiếp từ Bổ đề~\ref{lemma:p14}, ta có chặn trên cho từng cụm:
...
% Định nghĩa tập $G_2(P^*_i) = \{x \in P^*_i : \delta^2(x, c^*_i) \leq 2\delta^2(P^*_i, c^*_i) / |P^*_i|\}$. Theo Bổ đề \ref{lemma:4}, ta biết rằng ít nhất một nửa số điểm trong cụm tối ưu là điểm tốt :
...
Hệ quả này tương tự Hệ quả \ref{cor:1}, chứng minh tương tự.
...
Hệ quả  này tương tự Bổ đề \ref{lemma:10}, chứng minh tương tự.
...
Dựa trên Hệ quả \ref{cor:4} trong bài báo , tâm $c_i$ được chọn bởi bộ ước lượng thỏa mãn điều kiện về chi phí với xác suất cao:
...
\textbf{3. Áp dụng bất đẳng thức tam giác nới lỏng \ref{thm:relaxed_triangle}}
...
% TODO prove relaxed
Áp dụng bất đẳng thức tam giác nới lỏng \ref{thm:relaxed_triangle} dạng $(a+b)^2 \leq (1 + \frac{1}{\lambda})a^2 + (1 + \lambda)b^2$. Ở đây ta chọn $\lambda = 2$ để tối ưu hóa các hệ số theo bài báo:
...
\textbf{2. Áp dụng bất đẳng thức tam giác nới lỏng \ref{thm:relaxed_triangle}}
...
Theo giả thiết bài toán và Bổ đề \ref{lemma:12}:
\begin{itemize}
    ...
    \item Khoảng cách giữa các tâm (từ Bổ đề \ref{lemma:12}): $\delta^2(\overline{I_i}, c^*_i) \leq \frac{9\delta^2(P^*_i, c^*_i)}{(1 - (3+\epsilon)\alpha)m_i}$.
    \item Chi phí trong cụm: $\delta^2(A_i, c^*_i) \leq \delta^2(P^*_i, c^*_i)$ và $\delta^2(B_i, \overline{I_i}) \leq 4\delta^2(P^*_i, c^*_i)$.
\end{itemize}

Thay thế các giá trị này vào phương trình Bổ đề \ref{lemma:13}:
\begin{align*}
...
Sử dụng kết quả từ Bổ đề \ref{lemma:13}:
\[ \delta^2(Q_i, \overline{I_i}) \leq \delta^2(Q_i, c^*_i) + \left( 5\sqrt{\alpha} + \frac{36(\sqrt{\alpha} + \alpha)}{1-(3+\epsilon)\alpha} \right) \delta^2(P^*_i, c^*_i) \]
\item \textbf{Chặn trên cho $R_i$ (âm tính giả của dự đoán)}
Áp dụng bất đẳng thức tam giác nới lỏng \ref{thm:relaxed_triangle} với $\lambda = \sqrt{\alpha}$ cho mỗi $p \in R_i$:
\begin{align*}
...
Theo Bước 6 và 7 của Thuật toán \ref{alg:kmedian_sampling}, không gian xung quanh các mẫu được phân rã thành các lưới con. Với xác suất cao, tồn tại một điểm ứng viên $u' \in U'_i$ nằm rất gần tâm tối ưu $c^*_i$. Cụ thể, dựa trên kích thước lưới con $(1-\alpha)\alpha\epsilon_1 l_i / \sqrt{d}$, khoảng cách này được chặn bởi:
...
Dựa vào tính chất tối ưu trong Bước 9 của Thuật toán \ref{alg:kmedian_sampling}, $u_i$ là điểm tối thiểu hóa chi phí k-median đối với tập lân cận của nó. Theo Hệ quả \ref{cor:5} đã chứng minh trước đó, với xác suất cao, chi phí này bị chặn bởi:
...
Thành phần 1 (Tập được chọn):
    Theo Hệ quả \ref{cor:5}, với xác suất cao, chi phí của tập được chọn thỏa mãn:
...
Thay thế $|A_i| + |B_i| \leq 2\alpha m_i$ và khoảng cách tâm $\delta(u_i, c^*_i)$ từ Bổ đề \ref{lemma:15}:
\[ \delta(u_i, c^*_i) \leq \frac{(2+\alpha\epsilon)\delta(P^*_i, c^*_i)}{(1-2\alpha)m_i} \]
Ta có:
\begin{align*}
...
Xét tập hợp $Q_i = P^*_i \cap P_i$. Áp dụng trực tiếp kết quả từ Bổ đề \ref{lemma:16}, ta có chặn trên cho chi phí của tập này đối với tâm $u_i$:
...
Sử dụng kết quả từ Bổ đề \ref{lemma:15} cho khoảng cách giữa hai tâm $\delta(u_i, c^*_i) \leq \frac{(2 + \alpha\epsilon)\delta(P^*_i, c^*_i)}{(1 - 2\alpha)m_i}$.
