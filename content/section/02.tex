...
\textbf{Bước 2: Áp dụng Bất đẳng thức Chernoff}
...
\textbf{Bước 2: Chuẩn bị áp dụng Bất đẳng thức Chernoff}
...
\textbf{1. Phân tích Chất lượng Phân cụm}
...
\textbf{2. Phân tích Xác suất Thành công}
...
\begin{enumerate}
    \item \textbf{Định nghĩa Biến ngẫu nhiên:} Xét quá trình lấy mẫu $S_{ij}$ từ $P_{ij}$. Gọi biến ngẫu nhiên $X_p$, bằng 1 nếu điểm $p \in P_{ij}$ được chọn vào $S_{ij}$ và 0 nếu ngược lại. Tổng số điểm thuộc một tập con bất kỳ $A \subseteq P_{ij}$ rơi vào mẫu là $X = \sum_{p \in A} X_p$.
...
\textbf{3. Phân tích Thời gian Chạy}

Thời gian chạy của thuật toán được phân tích theo từng giai đoạn xử lý:

\begin{itemize}
    \item \textbf{Xây dựng Ứng viên (Bước 3-7):} 
    Việc lấy mẫu $U_{ij}$ mất thời gian $O(1)$. Thuật toán lặp qua $O(\log(m\Delta_{\max}))$ giá trị độ dài khoảng, mỗi lần tạo ra tập ứng viên. Tổng số lượng ứng viên được tạo ra là $O(\epsilon^{-1} \log(m\Delta_{\max}) \log(kd))$. 
    ...
    \item \textbf{Lựa chọn Cuối cùng (Bước 12):}
