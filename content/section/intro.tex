...
% BEGIN PB
\textbf{Fast-Filtering (Tác giả)} & $1 + 6\sqrt{\alpha} + \frac{36(\sqrt{\alpha} + \alpha)}{1-(3+\epsilon)\alpha} + \frac{9(\sqrt{\alpha}+\alpha)}{(1-\alpha)(1-(3+\epsilon)\alpha)}$ & $(0, 1/3-\epsilon)$ & $O(md) + \tilde{O}(\epsilon^{-5}kd/\alpha)$ \\ \hline
% END PB
\end{tabular}
}
\end{table}
...
% BEGIN PB
Nếu so sánh tỷ lệ xấp xỉ theo lý thuyết, 2 thuật toán đầu của tác giả không tốt hơn so với các thuật toán sắp xếp, trừ Fast-Filtering. Tuy nhiên, cả 3 đều cải thiện độ phức tạp thời gian.
...
\end{figure}
\begin{enumerate}
    \item Đường màu xanh lá cây (Thuật toán Phân vùng và Sắp xếp)
    \item Đường màu xanh lục bảo (Thuật toán Sắp xếp)
    \item Đường màu xanh dương (Thuật toán Fast-Estimation)
    \item Đường màu xám (Thuật toán Fast-Filtering)
\end{enumerate}

Có thể thấy Fast-Filtering tỷ lệ xấp xỉ càng thấp thấp hơn khi càng gần $1/3$
