...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1.8, >=Stealth]
        ...
        \begin{scope}[shift={(0, -1.8)}]
            ...
            \fill[badred] (1, 0) circle (2pt) node[right, black, font=\small] {: Điểm nằm xa};
        \end{scope}

    \end{tikzpicture}
    \caption{\hl{Bổ đề 4 trên không gian 1 chiều: Các điểm dữ liệu phân bố trên trục số, tập trung chủ yếu quanh trọng tâm trong một phạm vi ngưỡng xác định.}}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=2, >=Stealth]
        % 1. Định nghĩa màu sắc
        \definecolor{setQ}{RGB}{31, 119, 180}    % Xanh dương cho Q
        \definecolor{sampleI}{RGB}{44, 160, 44}  % Xanh lá cho mẫu I
        \definecolor{errorRed}{RGB}{214, 39, 40} % Đỏ cho sai số

        % 2. Vẽ trục tọa độ R
        \draw[->, thick] (-2.5, 0) -- (2.5, 0) node[right] {$\mathbb{R}$};

        % 3. Vẽ các điểm của tập Q_ij (các điểm xanh mờ)
        \foreach \x in {-1.8, -1.4, -1.1, -0.7, -0.3, 0.2, 0.6, 1.0, 1.4, 1.9} {
            \fill[setQ!30] (\x, 0) circle (1.5pt);
        }
        
        % 4. Vẽ các điểm được chọn vào mẫu I_{ij} (các điểm xanh lá đậm)
        \foreach \x in {-1.4, -0.3, 0.6, 1.4} {
            \fill[sampleI] (\x, 0) circle (2pt);
            \draw[sampleI, thin] (\x, 0) circle (3pt); % Khoanh tròn điểm mẫu
        }

        % 5. Đánh dấu Trọng tâm thực Q_bar (màu đen)
        \coordinate (Qbar) at (0.02, 0); % Trọng tâm tính toán từ các điểm trên
        \draw[thick] (Qbar) -- (Qbar |- 0, 0.4);
        \fill[black] (Qbar) circle (2pt);
        \node[above, font=\large] at (0.02, 0.4) {$\overline{Q_{ij}}$};

        % 6. Đánh dấu Trọng tâm mẫu I_bar (màu đỏ, lệch một chút)
        % Tính toán I_bar từ {-1.4, -0.3, 0.6, 1.4} = 0.075
        \coordinate (Ibar) at (0.35, 0); 
        \draw[thick, errorRed] (Ibar) -- (Ibar |- 0, 0.4);
        \fill[errorRed] (Ibar) circle (2pt);
        \node[above, errorRed, font=\large] at (0.35, 0.4) {$\overline{I_{ij}}$};

        % 7. Minh họa khoảng cách sai lệch (Bound)
        \draw[<->, errorRed, thick] (Qbar |- 0, 0.2) -- (Ibar |- 0, 0.2) 
            node[midway, below, font=\tiny, yshift=-2pt] {Sai số chặn bởi $\alpha, \epsilon$};

        % 8. Chú thích các tập hợp
        \draw [thick, decoration={brace, mirror, amplitude=6pt}, decorate] 
            (-1.9, -0.2) -- (2.0, -0.2) 
            node [pos=0.5, anchor=north, yshift=-6pt, font=\small] {Tập giao $Q_{ij}$};

        % 9. Bảng chú thích (Legend)
        \begin{scope}[shift={(1.0, -0.8)}]
            \fill[setQ!30] (0,0) circle (2pt) node[right, black, font=\scriptsize] {: Điểm trong $Q_{ij}$};
            \fill[sampleI] (0,-0.2) circle (2pt) node[right, black, font=\scriptsize] {: Điểm mẫu $I_{ij}$};
            \node[right, errorRed, font=\scriptsize] at (-0.1,-0.4) {$\bullet$ : Khoảng cách giữa hai trọng tâm};
        \end{scope}

    \end{tikzpicture}
    \caption{Trực quan hóa Bổ đề 6: Trọng tâm của tập mẫu $I_{ij}$ (màu đỏ) nằm rất gần trọng tâm thực tế của tập $Q_{ij}$ (màu đen). Khoảng cách này được kiểm soát bởi tham số nhiễu $\alpha$ và sai số cho phép $\epsilon$.}
\end{figure}
