...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1.8, >=Stealth]
        ...
        \definecolor{sampleI}{RGB}{44, 160, 44}  % Xanh lá cho mẫu I (liên tiếp)
        ...
        % 3. Tập P_ij (20 điểm - Tọa độ không cách đều nhau)
        % Danh sách x: 
        \def\pts{-3.4, -3.1, -2.7, -2.3, -2.1, -1.6, -1.3, -1.1, -0.7, -0.5, -0.1, 0.2, 0.4, 0.9, 1.2, 1.6, 2.1, 2.5, 2.9, 3.3}
        
        % Vẽ nền xám P_ij
        \foreach \x in \pts {
            \fill[pointgray] (\x, 0) circle (1.85pt);
        }
        
        % 4. Vẽ tập Q_ij (Xanh dương - Rải rác - chọn 14/20 điểm ~ 70%)
        \foreach \x in {-3.4, -3.1, -2.3, -2.1, -1.3, -1.1, -0.7, -0.5, -0.1, 0.4, 0.9, 1.6, 2.1, 2.9} {
            \fill[setQ!60] (\x, 0) circle (2.3pt);
        }
        
        % 5. Vẽ tập mẫu I_ij (Xanh lá - Liên tiếp - 14 điểm liên tiếp từ vị trí số 4 đến 17)
        \foreach \x in {-2.3, -2.1, -1.6, -1.3, -1.1, -0.7, -0.5, -0.1, 0.2, 0.4, 0.9, 1.2, 1.6, 2.1} {
            \fill[sampleI] (\x, 0) circle (2.2pt);
            \draw[sampleI, thin] (\x, 0) circle (3.2pt);
        }

        % 6. Trọng tâm Q_bar (Tầng thấp - 0.45)
        \coordinate (Qbar) at (-0.05, 0); 
        \draw[thick] (Qbar) -- (Qbar |- 0, 0.45);
        \fill[black] (Qbar) circle (2.8pt);
        \node[above, font=\large] at (-0.05, 0.45) {$\overline{Q_{ij}}$};

        % 7. Trọng tâm I_bar (Tầng cao - 0.75)
        \coordinate (Ibar) at (0.3, 0); 
        \draw[thick, errorRed] (Ibar) -- (Ibar |- 0, 0.75);
        \fill[errorRed] (Ibar) circle (2pt);
        \node[above, errorRed, font=\large] at (0.3, 0.75) {$\overline{I_{ij}}$};

        % 8. Mũi tên sai số
        \draw[<->, errorRed, thick] (Qbar |- 0, 0.25) -- (Ibar |- 0, 0.25);

        % 9. Dấu ngoặc nhọn chú thích
        % Ngoặc P_ij dưới trục
        \draw [thick, decoration={brace, mirror, amplitude=8pt}, decorate] 
            (-3.5, -0.4) -- (3.4, -0.4) 
            node [pos=0.5, anchor=north, yshift=-10pt, font=\small] {Tập dữ liệu cụm $P_{ij}$ ($m_i$ điểm)};
            
        % Ngoặc I_ij trên cùng (Tầng 1.1)
        ...
        \begin{scope}[shift={(0.8, -1.4)}]
            \fill[setQ!60] (0,0) circle (2pt) node[right, black, font=\scriptsize] {: Điểm trong $Q_{ij}$ (không liên tiếp)};
            \node[right, errorRed, font=\scriptsize] at (-0.1,-0.2) {$\leftrightarrow$ : Khoảng cách giữa hai trọng tâm};
        \end{scope}

    \end{tikzpicture}
    \caption{\hl{Bổ đề 6: Khoảng cách giữa trọng tâm mẫu $I_{ij}$ và trọng tâm thực $Q_{ij}$. $\alpha = 30\%$.}}
\end{figure}
